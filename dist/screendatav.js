(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.screenDatav = factory(global.vue, global.echarts));
  ...
  var css_248z$3 = ".SvgLoadingWrapper {\n  width: 100%;\n  height: 100%;\n  display: flex;\n  flex-direction: column;\n  justify-content: center;\n  align-items: center;\n}\n.SvgLoadingWrapper .LoadingTextWrapper {\n  margin-top: 20px;\n  margin-left: 15px;\n  color: #fff;\n}";
  styleInject(css_248z$3);

  script$3.__file = "src/component/Loading/index.vue";
  ...
  const props = __props;
  const HEADERDATA = vue.ref([]);//处理后的header数据
  const HEADERSTYLE = vue.ref([]);//处理后的header样式数据
  const COLUMWIDTH = vue.ref([]);//动态计算的宽度
  const ROWSDATA = vue.ref([]);//处理后的Rowsdata
  const ROWSHEIGHT = vue.ref([]);//动态计算的高度
  const ROWSNUMBER = vue.ref(0);//你需要展示的行数
  const ROWSTYLE = vue.ref([]);//内容列样式
  const ROWBG = vue.ref([]);//行背景样式
  const ALINGS = vue.ref([]);//是否居中显示
  const CURRENTROWDATA = vue.ref([]);//实际展示的数据条数据
  const CURRENTINDEX = vue.ref(0);//当前需要展示动画的位置
  ...
  return (_ctx, _cache) => {
    return (vue.openBlock(), vue.createElementBlock("div", {
      ...
      vue.createCommentVNode(" 搭建头部组件 "),
      vue.createCommentVNode(" 需要实现外部组件自定义样式与内容和本组件动态计算宽度"),
      vue.createElementVNode("div", _hoisted_1, [
        vue.createElementVNode("div", {
          ...
          (vue.openBlock(true), vue.createElementBlock(vue.Fragment, null, vue.renderList(HEADERDATA.value, (item, index) => {
            return (vue.openBlock(), vue.createElementBlock("div", {
              class: "headertext",
              innerHTML: item,
              key: item + index,
              style: vue.normalizeStyle({ width: `${COLUMWIDTH.value[index]}px`, ...HEADERSTYLE.value[index] }),
              align: ALINGS.value[index]
            }, null, 12 /* STYLE, PROPS */, _hoisted_2))
          }), 128 /* KEYED_FRAGMENT */))
        ], 4 /* STYLE */)
      ]),
      (vue.openBlock(true), vue.createElementBlock(vue.Fragment, null, vue.renderList(CURRENTROWDATA.value, (item, index) => {
        return (vue.openBlock(), vue.createElementBlock("div", {
          class: "baseScorelist",
          key: index + item[0]
        }, [
          vue.createElementVNode("div", {
            class: "itemWrapper",
            style: vue.normalizeStyle({ height: `${ROWSHEIGHT.value[index]}px`, backgroundColor: index % 2 === 0 ? ROWBG.value[0] : ROWBG.value[1], color: `${props.config.rowColor}` })
          }, [
            (vue.openBlock(true), vue.createElementBlock(vue.Fragment, null, vue.renderList(item, (iten, index) => {
              return (vue.openBlock(), vue.createElementBlock("div", {
                class: "item",
                innerHTML: iten,
                key: index + item,
                align: ALINGS.value[index],
                style: vue.normalizeStyle({ width: `${COLUMWIDTH.value[index]}px`, ...ROWSTYLE.value[index], fontSize: `${props.config.rowFontsize}px`, })
