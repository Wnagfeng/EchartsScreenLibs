(function (global, factory) {
  ...
  const AVERAGEWIDTH = vue.ref([]);
  const ALINGS = vue.ref([]);
  const DATA = vue.ref([]);
  const COLUMSTYLE = vue.ref([]);
  const COLUMNHEIGHT = vue.ref([]);
  const ODDROWBGC = vue.ref('');
  const EVENROWBGC = vue.ref('');
  let CURRENTINDEX = vue.ref(0);
  const CURRENTDATA = vue.ref([]);
  const AVERAGEHEIGHT = vue.ref(0);
  const props = __props;
  // 处理头部数据
      ...
      vue.createElementVNode("div", _hoisted_1, [
        vue.createElementVNode("div", {
          ...
          (vue.openBlock(true), vue.createElementBlock(vue.Fragment, null, vue.renderList(HEADERDATA.value, (item, index) => {
            return (vue.openBlock(), vue.createElementBlock("div", {
              class: "HeaderItem",
              style: vue.normalizeStyle({ ...HEADERSTYLE.value[index], width: `${AVERAGEWIDTH.value[index]}px` }),
              align: ALINGS.value[index]
            }, vue.toDisplayString(item), 13 /* TEXT, STYLE, PROPS */, _hoisted_2))
